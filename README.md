...
**Phase 3.8 status:** structured report-state v1 remains **frozen and calibration-only** — schema, deterministic per-field extractor, 59-case golden set, and SHA-256 lock are unchanged. The independent audit paused PR C because of material enum, joint-state, and quote/hypothetical gaps. A bounded [v1.1 amendment is now preregistered](docs/PHASE3_8_STRUCTURED_REPORT_STATE_V1_1_PREREGISTRATION.md), but it is **not implemented and authorizes no Run 001/002 diagnostic processing or empirical adoption**. Temporary empirical default remains classifier-v2. See the original [Phase 3.8 preregistration](docs/PHASE3_8_STRUCTURED_REPORT_STATE_PREREGISTRATION.md) and [v1 calibration audit](docs/reports/PHASE3_8_STRUCTURED_REPORT_STATE_CALIBRATION_AUDIT.md).
